...
class Reader(nn.Module):
    ...
    def _forward(self, input_ids, attention_mask, toke_type_ids: T):
        sequence_output, _pooled_output, _hidden_states = self.encoder(input_ids.long(), toke_type_ids.long(), attention_mask)
...
def _create_question_passages_tensors(
    ...
    if positives_num + negatives_num < total_size:
        empty_negatives = [empty_ids.clone().view(1, -1).float() for _ in range(total_size - (positives_num + negatives_num))]
        empty_token_type_ids = [
            empty_ids.clone().view(1, -1).float() for _ in range(total_size - (positives_num + negatives_num))
        ]

        input_ids = torch.cat([input_ids.float(), *empty_negatives], dim=0)
        toke_type_ids = torch.cat([toke_type_ids.float(), *empty_token_type_ids], dim=0)
